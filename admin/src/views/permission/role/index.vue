<template>
  <div class="role-lists">
    ...
    <auth-api-popup v-if="showAuthApi" ref="authApiRef" @success="getLists" @close="showAuthApi = false"/>
  </div>
</template>
...
import AuthApiPopup from './authapi.vue'
import AuthPopup from './auth.vue'

const editRef = shallowRef<InstanceType<typeof EditPopup>>()
const authRef = shallowRef<InstanceType<typeof AuthPopup>>()
const authApiRef = shallowRef<InstanceType<typeof AuthApiPopup>>()
const showEdit = ref(false)
const showAuth = ref(false)
const showAuthApi = ref(false)
...
const handleAuthApi = async (data: any) => {
  showAuthApi.value = true
  await nextTick()
  authApiRef.value?.open()
  authApiRef.value?.setFormData(data)
}

// 删除角色
const handleDelete = async (id: string) => {
  await feedback.confirm('确定要删除？')
  await roleDelete({ids:[id]})
